 ...
 *
 * OpsExample and Helper are defined in src/main/scala and have their
...
class OpsAnnotationTest extends FunSuite:
  ...
  test("non-primitive argument automatically tracked - dependencies concatenated") {
    ...
    assertEquals(result, (OpsExample("Alice & Bob", "30+25"), ex1))
  }
  ...
  test("primitive argument automatically tracked - dependencies concatenated") {
    ...
    assertEquals(result, (OpsExample("Alicia", "30"), ex2))
  }
  ...
  test("different non-primitive type automatically tracked - Helper argument") {
    ...
    assertEquals(result, ("Alice says hello to cat", "Alice says hello to cat"))
  }

  test("tracked Helper + primitive String - both automatically tracked") {
    val person = OpsExample("Alice", "30")
  ...
  test("multiple different non-primitive types all automatically tracked") {
    ...
    assertEquals(result, ("Alice & Bob says hello to parrot", "Alice & Bob says hello to parrot", "Alice & Bob says hello to parrot"))
  }

  test("@unrestricted annotation opts out of tracking - second Helper not tracked") {
    val person = OpsExample("Alice", "30")
    ...
    assertEquals(result, ("Alice compares cat (tracked) with cat (not tracked)", trackedHelper))
  }