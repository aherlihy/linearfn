...
object OpsExtensionGenerator {
  ...
  private def generateExtensionCode(packageName: String, className: String, typeParams: Seq[Type.Param], methods: Seq[Defn.Def]): String = {
    ...
    // Type parameters for extension method declaration
    ...
    val extensionMethods = methods.map { method =>
      generateMethodExtension(className, typeParamNames, method)
    }.mkString("\n    ")
    ...
    s"""package $packageName
       ...
       |  extension $extensionTypeParams(p: RestrictedSelectable.Restricted[$fullClassName, D])
       |    $extensionMethods
       |""".stripMargin
  }

  private def generateMethodExtension(className: String, classTypeParams: Seq[String], method: Defn.Def): String = {
    val methodName = method.name.value

    // Full class type with type parameters
    val fullClassName = if (classTypeParams.isEmpty) {
      className
    } else {
      s"$className[${classTypeParams.mkString(", ")}]"
    }
